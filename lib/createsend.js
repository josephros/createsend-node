...
function CreateSend (auth, options) {
    ...
    var result = {
        ...
        requestWasRateLimited: function (result) {
            return result.headers['X-RateLimit-Remaining'] === 0 ? true : false;
        },
        getRateLimitData: function (result) {
            return {
                limited: this.requestWasRateLimited(result),
